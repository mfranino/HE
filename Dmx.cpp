...
Dmx Dmx::dmx;
volatile unsigned char Dmx::inBuff1[24], Dmx::inBuff2[24], Dmx::inVal[24];
volatile unsigned char Dmx::valid1, Dmx::valid2;
...
Dmx::Dmx()
{
    ...
    UCSR0A = 0;
    UCSR0B = BV( RXCIE ) | BV( RXEN ) | BV( UCSZ2 ) | BV( TXB8 );
                                                 // 9 bit rxen rxinten
    UCSR0C = BV( UCSZ1 ) | BV( UCSZ0 ); // 9 bit 1 stop
    // Dmx 2
    cbi( PORTD, 4 );
    sbi( DDRD, 4 );                 // input
    UBRR1L = ( MEGACLOCK / 4 - 1 ); // 250kbaud
    UCSR1A = 0;
    UCSR1B = BV( RXCIE ) | BV( RXEN ) | BV( TXEN ) | BV( UCSZ2 ) | BV( TXB8 ); 
                                                 // 9 bit rxen rxinten
    UCSR1C = BV( UCSZ1 ) | BV( UCSZ0 ); // 9 bit 1 stop
}
...
void Dmx::dmx1Int()
{
    ...
    if ( !data && !( statb & BV( RXB8 ) ) ) { // handle break here
        first1 = 1;
        scan1 = 0;
        read1 = 0;
        return;
